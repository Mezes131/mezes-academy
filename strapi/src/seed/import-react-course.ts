/**
 * Idempotent React course importer for Strapi.
 * Upserts by legacyId. All records stay draft until manually published.
 *
 * Usage (inside Strapi container / local strapi app):
 *   npx strapi console
 *   > const { importReactCourse } = require("./dist/src/seed/import-react-course");
...
export async function importReactCourse(strapi: Core.Strapi) {
  const candidates = [
    path.join(__dirname, "data", "react-course.json"),
    path.join(process.cwd(), "src", "seed", "data", "react-course.json"),
  ];
  const file = candidates.find((p) => fs.existsSync(p));
  if (!file) {
    throw new Error(
      `Missing react-course.json (looked in: ${candidates.join(", ")}). ` +
        "Run: npm run export:strapi-seed",
    );
  }
  ...
  strapi.log.info(`Imported React course draft (legacyId=${course.legacyId})`);
  return courseRow;
}

export default { importReactCourse };
